...
class HuberRegressionFunctionPage extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            ...
            // Hide/show results
            HuberRegression_show : false,
            HuberRegression_step2_show: false
    ...
    async handleSubmit(event) {
        ...
        this.setState({HuberRegression_show: false})
        ...
        API.get("huber_regression", {
            ...
            if (status === 'Success') {
                ...
                this.setState({HuberRegression_show: true})
            }
    ...
    async handleScatter(event) {
        ...
        this.setState({HuberRegression_step2_show: true})
    ...
    handleSelectFileNameChange(event){
        this.setState( {selected_file_name: event.target.value}, ()=>{
            ...
            this.state.HuberRegression_show=false
            this.state.HuberRegression_step2_show=false
        })
    }
                ...
                <Grid container direction="row">
                    <Grid item xs={4} sx={{ borderRight: "1px solid grey"}}>
                        <Typography variant="h5" sx={{ flexGrow: 1, textAlign: "center" }} noWrap>
                            Huber Regression Parameterisation
                        </Typography>
                        <hr/>
                        <form onSubmit={this.handleSubmit}>
                            ...
                            <FormControl sx={{m: 1, width:'90%'}} size={"small"}>
                                ...
                                />
                                <FormHelperText>Strength of the squared L2 regularization. Note that the penalty is equal to alpha * ||w||^2. Must be in the range [0, inf).</FormHelperText>
                            </FormControl>
                            <FormControl sx={{m: 1, width:'90%'}} size={"small"}>
                                <TextField
                                ...
                                />
                                <FormHelperText>Maximum number of iterations</FormHelperText>
                            </FormControl>
                            <FormControl sx={{m: 1, width:'90%'}} size={"small"}>
                                <TextField
                                ...
                                />
                                <FormHelperText>The parameter epsilon controls the number of samples that should be classified as outliers. The smaller the epsilon, the more robust it is to outliers. Epsilon must be in the range [1, inf).</FormHelperText>
                            </FormControl>
                        ...
                        <form onSubmit={this.handleProceed}>
                            <Button sx={{float: "right", marginRight: "2px"}} variant="contained" color="primary" type="submit"
                                    disabled={!this.state.HuberRegression_show}>
                                Proceed >
                            </Button>
                        </form>
                        ...
                        <div  style={{display: (this.state.HuberRegression_show ? 'block' : 'none')}}>
                            <hr style={{display: (this.state.HuberRegression_show ? 'block' : 'none')}}/>
                            <Typography variant="h5" sx={{ flexGrow: 1, textAlign: "center" }} noWrap>
                                Available Variables
                            </Typography>
                            ...
                            <div style={{ display: (this.state.HuberRegression_step2_show ? 'block' : 'none') }}>
                    ...
                    <Grid  item xs={8}>
                        ...
                        {/*<div style={{ display: (this.state.HuberRegression_show ? 'block' : 'none') }}>{this.state.coefficients}</div>*/}
                        {/*<div style={{ display: (this.state.HuberRegression_show ? 'block' : 'none') }}>{this.state.intercept}</div>*/}
                        {/*<div style={{ display: (this.state.HuberRegression_show ? 'block' : 'none') }}>{this.state.dataframe}</div>*/}
                        <hr style={{ display: (this.state.Huber_show ? 'block' : 'none') }}/>
                        <Box sx={{ width: '100%' }}>
                            ...
                            <TabPanel value={this.state.tabvalue} index={2}>
                                <div style={{display: (this.state.status === 'Success' ? 'block': 'none')}}>
                                    <div style={{display: (this.state.HuberRegression_show ? 'block' : 'none')}} dangerouslySetInnerHTML={{__html: this.state.df_scatter}} />
                                </div>
                            </TabPanel>
                        </Box>
                    </Grid>
...
export default HuberRegressionFunctionPage;
